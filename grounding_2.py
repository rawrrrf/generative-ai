...
# Define your datastores 
datastore_ids: List[str] = [
    "ra-selection-guide_1716893896897"
    # Add more datastore IDs as needed
]

# Create a list of tools, one for each datastore
tools = []
for datastore_id in datastore_ids:
    datastore = f"projects/{DATA_STORE_PROJECT_ID}/locations/{DATA_STORE_REGION}/collections/default_collection/dataStores/{datastore_id}"
    tool = Tool.from_retrieval(
        preview_grounding.Retrieval(preview_grounding.VertexAISearch(datastore=datastore))
    )
    tools.append(tool)

# Initialize the chat session and conversation memory 
model = GenerativeModel(MODEL_NAME)
chat = model.start_chat(response_validation=True)
...
while True:
    ...
    # Pass ALL tools to chat.send_message 
